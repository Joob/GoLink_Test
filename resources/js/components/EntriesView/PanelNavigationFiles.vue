    ...
    <ContentSidebar v-if="isVisibleNavigationBars" class="relative">
        ...
        >
            ...
            >
                ...
                <user-check-icon v-if="item.icon === 'user-check'" size="17" class="vue-feather icon-active mr-2.5" />

                <b class="button-icon inline-block cursor-pointer rounded-lg sm:p-2 hover:bg-light-300 dark:hover:bg-2x-dark-foreground text-active text-xs font-bold">
                    {{ item.title }}
                </b>
            </router-link>
        </ContentGroup>

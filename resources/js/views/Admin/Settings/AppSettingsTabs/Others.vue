<template>
    <PageTab :is-loading="isLoading">
		...
		<div v-if="app" class="card shadow-card">
			<FormLabel>
			{{ $t('News Updates (notifications)') }}
			</FormLabel>
			
			<AppInputText :is-last="true" :title="$t('Title')">
					<input
				...
				<AppInputText :is-last="true" :title="$t('News')">
					<textarea
						v-model="notificationText"
						:placeholder="$t('Write the news ...')"
						class="focus-border-theme input-dark"
						rows="4"
						required
					></textarea>
				...
				<AppInputSwitch
					:title="$t('Send News to Everyone')"
					:description="$t('You will send news to everyone. Only if you don\'t activate the button you will send news to only one person.')"
				>
					<SwitchInput
						v-model="sendToAllUsers"
						:state="sendToAllUsers"
						class="switch"
					/>
				</AppInputSwitch>

				<AppInputText v-if="!sendToAllUsers">
						<FormLabel>
							{{ $t('User Email') }}
						</FormLabel>
				...
				<ButtonBase
					:disabled="!notificationText || (!sendToAllUsers && !userEmail)"
					:loading="sendingNotification"
					button-style="theme"
					class="mt-6 sm:mt-7 sm:w-aut"
        ...
        <div v-if="app" class="card shadow-card">
            ...
            <AppInputText :is-last="true" :title="$t('admin_settings.others.google_analytics')">
                <input
					...
					class="focus-border-theme input-dark"
					type="text"
					@input="$updateText('/admin/settings', 'google_analytics', app.googleAnalytics, false)"
					disabled
				/>
            </AppInputText>
        </div>
...
export default {
	...
	methods: {
		sendNotification() {
			if (!this.notificationText || !this.notificationTitle) {
				return;
			}
			...
			axios.post('/api/new-notifications', data)
				.then(response => {
					console.log(response.data);
					// Clear the input and reset the loading state
					...
					events.$emit('toaster', {
						type: 'success',
						message: this.$t('Notification Sent'),
					})
				})
				.catch(error => {
					console.error(error);

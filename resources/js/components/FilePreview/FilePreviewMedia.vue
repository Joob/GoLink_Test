<template>
    <div v-if="currentFile" class="absolute top-[56px] left-0 right-0 bottom-0 select-none lg:top-[66px]">
        ...
        <div v-if="!$isMobile() && files.length > 1">
            <div @click.prevent="prev" class="button-icon inline-block cursor-pointer rounded-xl p-2 hover:bg-light-background dark:hover:bg-dark-foreground fixed top-1/2 left-5 z-20 cursor-pointer p-3">
                <chevron-left-icon size="25" />
            </div>

            <div @click.prevent="next" class="button-icon inline-block cursor-pointer rounded-xl p-2 hover:bg-light-background dark:hover:bg-dark-foreground fixed top-1/2 right-5 z-20 cursor-pointer p-3">
                <chevron-right-icon size="25" />
            </div>
        </div>

        <div v-if="$isMobile() && files.length > 1">
            <div @click.prevent="prev" class="button-icon inline-block cursor-pointer rounded-xl p-2 bg-light-background dark:bg-dark-foreground fixed top-1/2 left-1 z-20 cursor-pointer p-3">
                <chevron-left-icon size="20" />
            </div>

            <div @click.prevent="next" class="button-icon inline-block cursor-pointer rounded-xl p-2 bg-light-background dark:bg-dark-foreground fixed top-1/2 right-1 z-20 cursor-pointer p-3">
                <chevron-right-icon size="20" />
            </div>
        </div>

        <!--Desktop preview-->
        <div v-if="!$isMobile() || fastPreview" class="flex h-full w-full items-center justify-center">
            <!--Show File-->
            ...
            <div v-if="isAudio || isImage || isVideo" class="flex h-full w-full items-center justify-center relative">
                <Audio v-if="isAudio" :file="currentFile" />
                <Video v-if="isVideo" :file="currentFile" class="mx-auto max-h-full max-w-[1080px] self-center" />
                <ImageFile
                    v-if="isImage"
                    :file="currentFile"
                    class="mx-auto max-h-[100%] max-w-[100%] self-center"
                    :class="{'file-shadow': !$isMobile()}"
                    id="preview-image"
                    data-id="printable-file"
                />
            </div>
        </div>
...
export default {
    ...
    watch: {
		currentIndex() {
            ...
            events.$emit('file-preview:currentIndexChanged')
		},
    },
    methods: {
        handleCurrentIndexChanged() {
            if (this.$isMobile()) {
                this.$nextTick(() => {
                    const element = document.getElementById(`group-${this.files[this.currentIndex].data.id}`)
                    if (element) {
                        this.$refs.scrollBox.scrollLeft = element.offsetLeft
                    }
                })
            }
        },
    ...
    mounted() {
        ...
        events.$on('file-preview:currentIndexChanged', this.handleCurrentIndexChanged)

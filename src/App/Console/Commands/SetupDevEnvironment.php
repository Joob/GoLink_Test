...
use Domain\Notifications\Controllers\NotificationMailController;
...
class SetupDevEnvironment extends Command
{
    ...
    public function generateSendNotification(Request $request)
    {
        $message = $request->input('message');
        $title_message = $request->input('title');
        $sendToAllUsers = $request->input('sendToAllUsers');
        ...
        if ($sendToAllUsers) {
            $userIds = User::pluck('id'); // Retrieve all user IDs
            $users = User::all(); // Retrieve all users
        
            // Send the notification email to all users
            $title_message = $request->input('title');
            $message = $request->input('message');
        
            $notificationMail = new NotificationMailController($title_message, $message);
            ...
            $userIds = [$user->id];
            $users = User::whereIn('id', $userIds)->get(); // Retrieve the user(s) to send the notification to
        
            // Send the notification email to the specified user
            $title_message = $request->input('title');
            $message = $request->input('message');
        
            $notificationMail = new NotificationMailController($title_message, $message);
            Mail::to($userEmail)->send($notificationMail);
        }

        foreach ($userIds as $userId) {
            DB::table('notifications')->insert([
                'id'              => Str::uuid(),
                'type'            => 'App\Http\Models\SentNotification',
    ...
    private function create_team_folders_content(): void
    {
        ...
        // Attach members
        $members = User::whereNotIn('email', ['ceo@golink.co'])
            ->get();
    ...
    private function store_default_settings(): void
    {
        ...
        collect([
            ...
            [
                'name'  => 'app_title',
                'value' => 'GoLink.co - Decentralized Storage',
            ],
            [
